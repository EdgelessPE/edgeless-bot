...
export default async function (p: ProducerParameters): Promise<Result<ProducerReturned, string>> {
	...
	if (!fs.existsSync(iniPath)) {
		...
		const fileContent = ini.parse(fs.readFileSync(iniPath).toString()).PortableApps.comInstaller;
		if (!fileContent) {
			return new Err('Error:Can\'t preprocess ' + taskName + ':[PortableApps.comInstaller] not found in pac_installer_log.ini');
		}
		try {
			...
			return new Err('Error:Can\'t preprocess ' + taskName + ':can\'t modify pac_installer_log.ini');
		}
	}
