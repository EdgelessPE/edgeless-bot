...
export default async function (p: ProducerParameters): Promise<Result<ProducerReturned, string>> {
	...
	//循环判断安装完成
	const finishFilePath = path.join(readyDir, 'GoogleChromePortableDev/Data/PortableApps.comInstaller/license.ini');
	...
	for (let f of deleteList) {
		shell.rm('-rf', path.join(readyDir, 'GoogleChromePortableDev', f));
	}
