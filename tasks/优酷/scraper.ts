...
export default async function (): Promise<Result<ScraperReturned, string>> {
	const page=(await robustGet('https://youku.com/product/index')).unwrap() as string
	...
	const json=JSON.parse(jsonTextMatch[0].split(/window.__INITIAL_DATA__\s*=\s*/)[1].slice(0,-1))
